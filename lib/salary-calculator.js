...
export class SalaryCalculator {
  ...
  // CORECTIE: Formula se aplica pe SUMA TOTALA deducerilor (baza + copii + alte persoane)
  calculatePersonalDeduction(grossSalary, children = 0, dependents = 0, isBasicFunction = true) {
    ...
    const childDeduction = this.rules?.child_deduction || 100;
    const dependentDeduction = this.rules?.dependent_deduction || 0;
    ...
    // Calcul suma totala maxima deduceri
    // DP_Total_Maxim = (Deducere_Baza + (Nr_Copii * 100) + (Nr_Alte_Pers * valoare))
    ...
    if (grossSalary <= minSalary) {
      return totalMaxDeduction;
    }
    
    // Formula regresiva pentru venituri intre SalMin si SalMin + 2000 RON
    // DP_Final = Math.round(DP_Total_Maxim * (1 - (Brut - SalMin) / 2000))
  ...
  calculateStandard(grossSalary, options = {}) {
    ...
    // 1. CAS (25% din brut) - ROTUNJIRE CONTABILĂ
    ...
    // 3. CASS (10% din brut + tichete) - ROTUNJIRE CONTABILĂ
    const cass = Math.round((grossSalary + voucherValue) * cassRate);
    
    // 4. Deducere personală TOTALĂ (bază + copii + alte persoane) cu formula regresivă
    const personalDeduction = this.calculatePersonalDeduction(grossSalary, children, dependents, isBasicFunction);
    
    // 5. Deducere tineri sub 26 ani
    const youthDeduction = this.calculateYouthDeduction(grossSalary, age, isBasicFunction);
    
    // 6. Venit impozabil = Brut - CAS - CASS - Deduceri
    const totalDeductions = personalDeduction + youthDeduction;
    const taxableIncome = Math.max(0, grossSalary - cas - cass - totalDeductions);
    
    // 7. Impozit pe venit (10% din venitul impozabil) - ROTUNJIRE CONTABILĂ
    const incomeTax = Math.round(taxableIncome * taxRate);
    
    // 8. Salariu NET = Brut - CAS - CASS - Impozit (toate deja rotunjite)
    const netSalary = grossSalary - cas - cass - incomeTax;
    
    // 9. Cost angajator: Brut + CAM (2.25%) + Tichete - ROTUNJIRE CONTABILĂ
    ...
    return {
      ...
      cass,
      incomeTax,
      personalDeduction, // Include deja baza + copii + alte persoane
      youthDeduction,
      childDeduction: 0, // Deja inclus in personalDeduction
      totalDeductions,
      taxableIncome,
      cam,
      totalCost,
  ...
  calculateIT(grossSalary, options = {}) {
    ...
    if (grossSalary <= threshold) {
      ...
      // Deducere personală se aplică pe partea care depășește pragul
      personalDeduction = this.calculatePersonalDeduction(taxableAmount, children, dependents, isBasicFunction);
      
      // Venit impozabil = (Brut - Prag IT) - Deducere
