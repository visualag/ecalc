      ...
      <main className="flex-grow container mx-auto px-4 py-6 max-w-6xl">
        ...
        <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200 flex gap-2 mb-6">
          <input 
            ...
            placeholder="Cauta alt oras..." className="flex-1 px-4 outline-none font-medium"
          />
          <button onClick={handleSearch} className="bg-blue-600 text-white px-6 py-2 rounded-xl font-bold hover:bg-blue-700">Cauta</button>
        </div>

        {weather && (
          <div className="space-y-6">
            {/* Azi - Design Compact Dashboard */}
  ...
  <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2">
    {weather.daily.time.map((date, index) => (
      <div key={date} className="bg-slate-50 p-4 rounded-3xl border border-transparent hover:border-blue-200 hover:bg-white transition-all text-center group">
          ...
          <div className="w-full bg-slate-100 h-1 rounded-full overflow-hidden">
            <div className="bg-blue-400 h-full" style={{ width: `${weather.daily.precipitation_probability_max[index]}%` }}></div>
                  ...
                  <div key={date} className="bg-slate-50 p-3 rounded-2xl border border-transparent hover:border-blue-200 hover:bg-white transition-all text-center">
                    ...
                    <p className="text-[10px] text-slate-400 font-bold mb-3">{new Date(date).toLocaleDateString('ro-RO', { day: 'numeric', month: 'short' })}</p>
                    <div className="text-xl font-black text-slate-800 leading-none">{Math.round(weather.daily.temperature_2m_max[index])}°</div>
                    <div className="text-[10px] font-bold text-slate-300 mb-2">{Math.round(weather.daily.temperature_2m_min[index])}°</div>
                    <div className="flex items-center justify-center gap-1 text-[9px] font-bold text-blue-400 bg-white rounded-lg py-1 border border-blue-50">
                      <Umbrella className="h-2 w-2" /> {weather.daily.precipitation_probability_max[index]}%
                    </div>
                  </div>
                ))}
              </div>
        ...
        {/* SEO - Orașe populare cu link-uri REALE */}
        <div className="mt-12">
