...
const getWeatherIcon = (code, isNight = false) => {
  ...
  switch (code) {
    case 0: return <Sun className={`${iconClass} text-yellow-500 fill-yellow-500/10`} />; // Clear
    case 1:
    case 2: return isNight ? <Moon className={`${iconClass} text-blue-300 fill-blue-300/10`} /> : <CloudSun className={`${iconClass} text-yellow-400 fill-yellow-400/10`} />; // Partly Cloudy
    case 3: return <Cloudy className={`${iconClass} text-slate-400 fill-slate-400/20`} />; // Cloudy
    case 45:
    case 48: return <Waves className={`${iconClass} text-slate-300`} />; // Fog
    case 51:
    case 53:
    case 55: return <Droplets className={`${iconClass} text-blue-400 fill-blue-400/20`} />; // Drizzle
    case 61:
    case 63:
    case 65: return <CloudRain className={`${iconClass} text-blue-500 fill-blue-500/30`} />; // Rain
    case 71:
    case 73:
    case 75: return <Snowflake className={`${iconClass} text-blue-100`} />; // Snow
    case 77: return <Snowflake className={`${iconClass} text-blue-200`} />; // Snow grains
    case 80:
    case 81:
    case 82: return <CloudRain className={`${iconClass} text-blue-600 fill-blue-600/40`} />; // Rain showers
    case 85:
    case 86: return <Snowflake className={`${iconClass} text-blue-300`} />; // Snow showers
    case 95: return <CloudLightning className={`${iconClass} text-yellow-500 fill-yellow-500/10`} />; // Thunderstorm
    default: return <CloudSun className={`${iconClass} text-slate-400`} />;
  }
};
      ...
      <div className="weather-module space-y-4">
        ...
        {theme === 'blue' ? (
          ...
          <div className="bg-[#171b28] text-white p-6 rounded-[4px] shadow-2xl overflow-x-hidden font-sans border border-slate-800">
            ...
            {/* 7-Day Forecast Grid */}
            <div className="grid grid-cols-7 gap-px bg-slate-800 rounded-[6px] overflow-hidden border border-slate-800 mb-8 p-0 shadow-2xl">
              {weather.daily.time.slice(0, 7).map((time, i) => {
                ...
                return (
                  <div key={time} className="bg-[#171b28] flex flex-col items-center pb-5 hover:bg-[#1a1d2d] transition-all group cursor-pointer relative border-r border-slate-800 last:border-0 min-h-[380px]">
                    {/* Header Band */}
                    <div className={`w-full py-2 text-center mb-6 transition-colors ${i === 0 ? 'bg-[#11141e] border-b border-white' : 'bg-[#242b3a] opacity-80'}`}>
                      <p className="text-[12px] font-black text-white">{dayName}</p>
                      <p className="text-[10px] font-bold text-slate-300 mt-0.5">{i === 0 ? 'Astăzi' : i === 1 ? 'Mâine' : dayDate}</p>
                    </div>

                    <div className="h-16 w-16 flex items-center justify-center mb-6 transform group-hover:scale-110 transition-transform">
                      {getWeatherIcon(weather.daily?.weather_code?.[i])}
                    </div>

                    <div className="space-y-1.5 mb-8 w-full px-2">
                    ...
                    <div className="text-[11px] space-y-3 mt-auto w-full px-2">
                      <div className="flex items-center gap-2">
                        <Triangle className="h-3 w-3 rotate-180 text-white fill-white" />
                        <span className="font-bold text-white tracking-tighter">⬅ {weather.daily?.wind_speed_10m_max?.[i] || 0} km/h</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Droplets className="h-4 w-4 text-white fill-current" />
                        <span className="font-bold text-white">{weather.daily?.precipitation_sum?.[i] || '-'} mm</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Sun className="h-4 w-4 text-yellow-500 fill-yellow-500/20" />
                        <span className="font-bold text-white">☀ {Math.round((weather.daily?.sunshine_duration?.[i] || 0) / 3600)} h</span>
                      </div>
                    </div>

                    {/* Intermediate Night Icons */}
                    {i < 6 && (
                      <div className="absolute -right-4 top-1/2 -translate-y-12 z-20 pointer-events-none hidden md:flex">
                        <div className="bg-[#11141e] rounded-full p-2 border border-slate-800 shadow-xl scale-75">
                          {getWeatherIcon(weather.daily?.weather_code?.[i], true)}
                        </div>
                      </div>
                    )}

                    {/* Predictability circles */}
                    <div className="flex justify-center gap-1 mt-6">
                      {[1, 2, 3, 4].map(n => (
                        <div key={n} className="w-3 h-3 rounded-full border-2 border-slate-600 relative overflow-hidden">
