          ...
          <div className="bg-[#171b28] text-white p-6 rounded-[4px] shadow-2xl overflow-x-hidden font-sans border border-slate-800">
            ...
            <div className="grid grid-cols-7 gap-px bg-slate-800 rounded-[6px] overflow-hidden border border-slate-800 mb-8 p-0 shadow-2xl">
              {weather.daily.time.slice(0, 7).map((time, i) => {
                ...
                return (
                      <p className="text-[12px] font-black text-blue-200">{dayName}</p>
            ...
            <div className="text-[11px] text-slate-400 space-y-3 mt-auto w-full px-2">
              ...
              <div className="flex items-center gap-2 group-hover:text-blue-400 transition-colors">
                <Droplets className="h-3.5 w-3.5 text-blue-400 fill-blue-400/20" />
                <span className="font-bold text-slate-100">{weather.daily?.precipitation_sum?.[i] || '-'} mm</span>
              </div>
              <div className="flex items-center gap-2 group-hover:text-yellow-500 transition-colors">
                <Sun className="h-3.5 w-3.5 text-yellow-500 fill-yellow-500/10" />
                <span className="font-bold text-slate-100">☀ {Math.round((weather.daily?.sunshine_duration?.[i] || 0) / 3600)} h</span>
              </div>
            </div>

            {/* Intermediate Night Icons logic (vremea de seara/noapte intre zile) */}
            {i < 6 && (
              <div className="absolute -right-4 top-1/2 -translate-y-12 z-20 pointer-events-none hidden md:flex">
                <div className="bg-[#11141e] rounded-full p-2 border border-slate-800 shadow-xl scale-75">
                  {getWeatherIcon(weather.hourly?.weather_code?.[(i * 24) + 22] || 0, true)}
                </div>
              </div>
            )}
